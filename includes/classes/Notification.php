...
class Notification {
    ...
    public function getNotifications($data, $limit) {
		...
		while($row = mysqli_fetch_array($query)) {
            ...
            $opened = $row['opened'];
			$style = ($row['opened'] == 'no') ? "border-bottom: 2px solid #5EAADA ;" : "";

            $return_string .= "<a id='left_alinks' href='" . $row['link'] . "'>
                                    <div id='left_links_container' style='" . $style . "'>
                                        <div id='icon_wrapper' class='float-left my-3 mr-1'>
                                            <img src='" . $user_data['profile_pic'] . "' class='rounded-circle w-100'>
                                        </div>
                                        <div id='text_wrapper'>
                                            <p class='timestamp_smaller'>" . $row['message'] . "<br>"
                                             . "<span class='text-primary'>" . $time_message . "</span>" .
                                             "</p>" . "
                                        </div>
                                    </div>
                                </a>";
